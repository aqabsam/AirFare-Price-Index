...
function outlierCount(values: number[]) {
...
export function calculateDataQuality(): DataQualityResponse {
  ...
  const qualityBySource = Object.fromEntries([...new Set(allRecords.map((snapshot) => snapshot.sourceType))].map((sourceType) => {
    ...
    const scores = records.map((snapshot) => snapshot.dataQualityScore).filter((score): score is number => score !== null && score !== undefined)
  ...
  const scores = allRecords.map((snapshot) => snapshot.dataQualityScore).filter((score): score is number => score !== null && score !== undefined)
