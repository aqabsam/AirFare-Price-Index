...
type DgcaLoadFactorObservation = {
  month: string
  airline: string
  percent: number
  page: number
...
type DgcaExtract = {
  ...
  monthlyPassengerLoadFactors: DgcaLoadFactorObservation[]
  fareObservations: Array<unknown>
}

export type DgcaAnalyticsResponse = {
  available: boolean
  source: 'DGCA-DATA.pdf'
  report: DgcaExtract['report'] | null
  monthlyPassengerLoadFactors: DgcaLoadFactorObservation[]
  fareObservations: DgcaExtract['fareObservations']
  fareAnalysisMessage: string
}

const datasetUrl = new URL('../../data/dgca-data.json', import.meta.url)

function isValidObservation(value: unknown): value is DgcaLoadFactorObservation {
  if (!value || typeof value !== 'object') return false
  const observation = value as Partial<DgcaLoadFactorObservation>
...
function unavailableResponse(): DgcaAnalyticsResponse {
  return {
    available: false,
    source: 'DGCA-DATA.pdf',
    report: null,
...
export async function getDgcaAnalytics(): Promise<DgcaAnalyticsResponse> {
  try {
    const dataset = JSON.parse(await readFile(fileURLToPath(datasetUrl), 'utf8')) as Partial<DgcaExtract>
