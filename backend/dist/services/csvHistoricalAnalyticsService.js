...
function readField(record, header) {
...
function datedFareMetric(date, records, indices) {
    const fares = records.map((record) => record.fare);
    const routeMeans = new Map();
    for (const record of records)
        routeMeans.set(record.routeKey, [...(routeMeans.get(record.routeKey) ?? []), record.fare]);
...
function emptyResponse(files, sourceRows, uniqueRows, duplicateRows, quality) {
    return {
...
export async function getCsvHistoricalAnalytics() {
    ...
    for (const record of allRecords) {
        ...
        if (existing)
            existing.sourceFiles.add(parsedFiles.find((file) => file.records.includes(record))?.summary.name ?? '');
        else {
    ...
    for (const record of uniqueRows) {
        ...
        if (originValue && !origin || destinationValue && !destination)
            invalidRouteRows += 1;
        ...
        if (!date || !origin || !destination || !fare || !currency)
            continue;
        ...
        if (actualFare && predictedFare)
            benchmarks.push({ actual: actualFare, predicted: predictedFare });
    ...
    if (!candidates.length)
        return emptyResponse(files, sourceRows, uniqueRows.length, duplicateRows, quality);
    ...
    for (const routeKey of routeKeys) {
        ...
        if (firstRecords.length)
            routeBaselines.set(routeKey, firstRecords.reduce((sum, record) => sum + record.fare, 0) / firstRecords.length);
    }
...
function calculateBenchmark(pairs) {
    if (pairs.length < 2)
        return { sampleCount: pairs.length, mae: null, rmse: null, mape: null, correlation: null };
