...
const configuredWeights = (() => {
    try {
        const parsed = JSON.parse(process.env.DGCA_ROUTE_WEIGHTS ?? '{}');
...
function matchesFilters(snapshot, origin, destination, departureDate) {
...
function baselinePrice(snapshots) {
...
function withIndex(points, baseline) {
    return points.map((point) => ({
        ...point,
        airfareIndex: indexForPrice(point.price, baseline),
    }));
}
function buildTrendSeries(snapshots, baseline) {
    const grouped = new Map();
    for (const snapshot of snapshots) {
...
function buildDailyIndex(snapshots, baseline) {
    const grouped = new Map();
    for (const snapshot of snapshots) {
...
function buildHeatmap(snapshots, baseline) {
...
function periodStart(dateString, period) {
...
function buildPeriodIndex(snapshots, period, baseline) {
    const grouped = new Map();
    for (const snapshot of snapshots) {
    ...
    const points = [...grouped.entries()].map(([periodStartValue, records]) => {
        const routeGroups = new Map();
        for (const record of records)
            routeGroups.set(record.routeKey, [...(routeGroups.get(record.routeKey) ?? []), record]);
...
function buildSourceComparison(snapshots) {
    const grouped = new Map();
    for (const snapshot of snapshots)
        grouped.set(snapshot.sourceId ?? snapshot.source, [...(grouped.get(snapshot.sourceId ?? snapshot.source) ?? []), snapshot]);
...
export function calculateRouteIndex(origin, destination, departureDate) {
...
export function calculateFareAnalytics(origin, destination, departureDate) {
    const snapshots = fareStore
        .getSnapshots()
        .filter((snapshot) => matchesFilters(snapshot, origin, destination, departureDate));
    const baseline = baselinePrice(snapshots);
