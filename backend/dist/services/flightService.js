...
import { getDuffelAccessToken, searchDuffelOffers } from '../providers/duffel.js';
...
const CURRENCY_TO_INR = { INR: 1, USD: 95.4, EUR: 111.28, GBP: 130.06 };
...
function comparablePriceInINR(offer) {
    return offer.price * (CURRENCY_TO_INR[offer.currency.trim().toUpperCase()] ?? 1);
}
export function hasMatchingFlightCarrier(offer) {
...
function recordRejection(rejections, reason) {
    if (rejections)
        rejections[reason] = (rejections[reason] ?? 0) + 1;
}
export function canonicalizeScrapedOffer(snapshot, rejections) {
    const reject = (reason) => {
        recordRejection(rejections, reason);
        return null;
    };
    if (!snapshot.airline?.trim())
        return reject('missing airline');
    if (!snapshot.flightNumber?.trim())
        return reject('missing flight number');
    if (!snapshot.origin?.trim() || !snapshot.destination?.trim())
        return reject('missing route');
    if (!snapshot.departureDate?.trim())
        return reject('missing travel date');
    const carrier = verifiedIndianAirline(snapshot.airline, snapshot.airlineCode ?? '');
    if (!carrier)
        return reject('invalid or conflicting Indian airline');
    ...
    if (!/^[A-Z0-9]{2,3}\d{2,5}$/.test(flightNumber))
        return reject('invalid flight number format');
    if (!flightNumber.startsWith(airlineCode))
        return reject('flight number carrier mismatch');
    const origin = snapshot.origin.trim().toUpperCase();
    const destination = snapshot.destination.trim().toUpperCase();
    if (!/^[A-Z]{3}$/.test(origin) || !/^[A-Z]{3}$/.test(destination))
        return reject('invalid airport code');
    ...
    if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(departureTime))
        return reject('invalid departure time');
    if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(arrivalTime))
        return reject('invalid arrival time');
    const duration = snapshot.duration?.trim() || (snapshot.durationMinutes && snapshot.durationMinutes > 0
        ...
        : '');
    if (!/^\d+h\s+\d{2}m$/i.test(duration))
        return reject('invalid or missing duration');
    const stops = Number(snapshot.stops);
    if (!Number.isInteger(stops) || stops < 0)
        return reject('invalid or missing stops');
    const sourceType = snapshot.sourceType ?? 'airline';
    if (sourceType !== 'airline' && sourceType !== 'ota')
        return reject('invalid source type');
    return {
...
function sourceUrl(scraper, input) {
...
async function withSourceTimeout(promise, sourceName) {
    let timeoutHandle;
    try {
...
async function searchLiveAirlineOffers(input) {
    ...
    try {
        browser = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('Live scraper startup timed out')), 3000)),
        ]).catch((error) => {
    ...
    try {
        const scraperResults = await Promise.all(scrapers.map(async (scraper) => {
            ...
            try {
                ...
                if (result.networkResponses.length)
                    console.info(`[Source ${sourceName}] XHR/fetch: ${result.networkResponses.join(' | ')}`);
