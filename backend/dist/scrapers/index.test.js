...
test('Google Flights result rows without a flight number report the exact parse rejection', () => {
    ...
    const snapshot = buildSnapshot('11:55 PM - 2:15 AM+1 Air India 2 hr 20 min BOM-DEL Nonstop INR 6,346', definition, { origin: 'BOM', destination: 'DEL', departureDate: '2026-09-26', adults: 1 }, 0, rejectionCounts);
...
test('search URL builders preserve configured hosts and exact route/date/passenger parameters', () => {
    ...
    try {
        ...
        if (original.ixigo === undefined)
            delete process.env.SCRAPER_IXIGO_URL;
        else
            process.env.SCRAPER_IXIGO_URL = original.ixigo;
        if (original.easemytrip === undefined)
            delete process.env.SCRAPER_EASEMYTRIP_URL;
        else
            process.env.SCRAPER_EASEMYTRIP_URL = original.easemytrip;
        if (original.spicejet === undefined)
            delete process.env.SCRAPER_SPICEJET_URL;
        else
            process.env.SCRAPER_SPICEJET_URL = original.spicejet;
    }
});
test('all configured Indian airline scrapers and OTAs are available to live search', () => {
    ...
    assert.deepEqual(airlineScrapers.map((scraper) => scraper.definition.id), allScrapers.map((scraper) => scraper.definition.id));
