...
function renderTemplate(value, route) {
    if (typeof value === 'string') {
        return value
            ...
            .replace(/\{\{\s*collectionDate\s*\}\}/gi, route?.collectionDate ?? '');
    }
    if (Array.isArray(value)) {
        return value.map((entry) => renderTemplate(entry, route));
    }
    if (value && typeof value === 'object') {
        return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, renderTemplate(entry, route)]));
    }
    return value;
}
function envSource(name, sourceType, urlEnv) {
    return {
        name,
        sourceType,
        kind: 'page',
...
export async function loadFareSourceConfigs() {
    ...
    if (configPath) {
        try {
            ...
            return entries.map(normalizeSourceEntry).filter((entry) => Boolean(entry));
        }
        catch {
            return [];
        }
...
export function renderFareSourceTemplate(value, route) {
