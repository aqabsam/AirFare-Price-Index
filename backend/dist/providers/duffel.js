...
function rawCarrierLabel(carrier) {
...
function logRawDuffelOffers(offers) {
    const marketingCounts = {};
    const operatingCounts = {};
...
export function normalizeDuffelOffer(offer, requestedRoute, diagnostics) {
...
function positiveOrZeroAmount(value) {
    const amount = Number(value);
    return Number.isFinite(amount) && amount >= 0 ? amount : null;
}
...
export async function searchDuffelOffers(input, onRawOfferCount, onOfferValidation) {
    ...
    try {
        ...
        offers.forEach((offer, index) => {
            ...
            for (const reason of Object.keys(diagnostics)) {
